## Spicer - port of Tomioka and Suzuki's SpicyMKL to R
##  Input:
##     K        : N x N x M array. the (i,j,m)-element contains the
##                (i,j)-element of the m-th kernel gram matrix.
##     yapp     : signal vector of length N (NOT a Nx1 or 1xN df).
##      Should be a factor for binary/multiclass classification!
...
##      history     :  contains history of primal objective, dual objective, 
...
##if you want the R functions
source("./Spicer-funcs.R")
## ##if you want the faster Rcpp code
##library(Rcpp)
##sourceCpp("./Spicer-funcs.cpp")
##use same name for both to avoid peppering code with if statements
##kernel.norm <- kernel_norm_cpp
...
spicer <- function(K,yapp,C,opt=list()) {
    ...
    if(is.factor(yapp) && length(levels(yapp))>2){
        ...
        res <- spicer.generic(K,yapp,C,opt)
    }
...
spicer.multiclass <- function(K,yapp,C,opt=list()){
    ...
    res <- foreach(i=1:ncol(combos))%do%{
        ...
        opts <- opt
        if(!is.null(opts$wghts)) opts$wghts <- opts$wghts[idx]
        spicer.generic(K[idx,idx,,drop=FALSE],factor(yapp[idx],levels=combos[,i]),C,opts)
...
spicer.generic<-function(K,yapp,C,opt=list()){

    if(is.null(opt$loss)) opt$loss <- if (length(unique(yapp))==2) "logit" else "square"

    if(is.null(opt$regname)) opt$regname <- "l1"

    
    if(is.null(opt$optname)) opt$optname <- "Newton" ## choices are Newton and BFGS
    
    ##if C has two components(elastic net), but the second one is zero,
    ##revert to l1-regularization
    if(length(C)==2 && C[2]==0){
        opt$regname="l1"
        C=C[1]
    }
    ...
    if(opt$loss%in%c("logit")){
        ...
        if(is.character(yapp)) yapp <- as.numeric(yapp)

        uniqYapp <- sort(unique(yapp))
        yapp[yapp==uniqYapp[1]] <- -1
        yapp[yapp==uniqYapp[2]] <- 1
        opt$classes <- if(exists("lyapp")) lyapp else uniqYapp
    }


    if(is.null(opt$tolOuter)) opt$tolOuter <- 0.001
    
    if(is.null(opt$tolInner)) opt$tolInner <- opt$tolOuter/1000

    if(is.null(opt$outerMaxIter)) opt$outerMaxIter <- 300

    if(is.null(opt$innerMaxIter)) opt$innerMaxIter <- 500

    if(is.null(opt$calpha)) opt$calpha <- 10

    if(is.null(opt$stopIneqViolation)) opt$stopIneqViolation <- FALSE

    if(is.null(opt$stopDualityGap)) opt$stopDualityGap <- TRUE

    if(is.null(opt$minIter)) opt$minIter <- 30

    if(is.null(opt$tolMinIter)) opt$tolMinIter <- 0.001    

    if(is.null(opt$display)) opt$display <- 2

    if(is.null(opt$includeSubW)) opt$includeSubW <- FALSE
    ...
    ##lagrangian for equality constraint on new variable z
    ## (rho in SpicyMKL Tomioka Suzuki JMLR 2011)
    rho <- -yapp*opt$wghts/2

    ##kernel weights divided by gamma 
    ##(alpha/gamma in SpicyMKL Tomioka Suzuki JMLR 2011)
    krnlWMod <- matrix(0,nrow=N,ncol=M)
    ##bias term intial estimate
    cb <- 0

    ##elastic net appears to be a lot closer to smoothness under the
    ##heuristic below, so no need to start with a very small gamma value
    ...
    ##input for proximal update for the kernel weights
    ##alpha/gamma + rho in SpicyMKL Tomioka Suzuki JMLR 2011
    ##remember, N=length(yapp)=length(rho)
    krnlWPrIn <- matrix(rho,N,M)+krnlWMod
    expand(kernel.norm(K,krnlWPrIn),c("wdot","normj"))
    ## this is the proximal of normj (not proximal of krnlWPrIn, which goes in the
    ## krnlWMod update equaiton)
    ...
    for (l in 1:opt$outerMaxIter){
        ...
        for (step in 1:opt$innerMaxIter){
         ...
         switch(opt$optname,
            ...
            ##special case for logit - need to confirm yrho is in the bounds
            ##specified for the logit convex conjugate
            ## (see SpicyMKL Tomioka Suzuki JMLR 2011 eqn 16)
            ...
            ##this update is needed to get the initial base obj function value
            ...
            ##compute step length in descent direction via Armijo's rule
            ##can be moved to a separate function, but calculation depends on too
            ##many arguments, some of which large - performance hit for
            ##slight increase in modularity
            dir <- rho -old$rho
            ...
            if (length(actdif)>0) {
                ...
                ##need to update portions of old$wdot and old$normj
                ##there might be a more efficient method to do it
                ##not even sure if these updates are necessary!! - to be confirmed
                tmp1 <- kernel.norm(K,old$krnlWPrIn,actdif)
                old$wdot[,actdif] <- tmp1$wdot
                old$normj[actdif] <- tmp1$normj
                rm(tmp1)
            }

            ##calculate normj and wdot including the directional component
            ##intermediate calcs for computing
            ##normj(alpha+rho+dir*steplen) from normj(alpha+rho)
            ##(the latter is what we had originally)
            dirNorm <- 0*normj
            dirDotWDot <- 0*normj
            ...
            while ( fval> old$fval + stepL*0.01*(dir%*%old$grad) &&
                       (grad%*%dir < 0.9*old$grad%*%dir || stepL==1)  &&
                           stepL > 0) {
            ## while ( fval> old$fval + stepL*0.01*(dir%*%old$grad) &&
            ##                stepL > 0) {
                ...
                ##all variables having rho as a component need to be updated
                ##prior to the funceval call
                normj <- 0*normj
            ...
            ## in case armijo rule cut step length
            ## update rho-dependent variables not used in Armijo step size calculation
            if (stepL!=1) {
                activeset <- which(pr>0)
                krnlWPrIn <- matrix(rho,length(rho),M)+krnlWMod
                wdot[,activeset] <- kernel.norm(K,krnlWPrIn,activeset)$wdot
                sumrho <- sum(rho)
                yrho <- yapp*rho
            }
            ...
            if (step==opt$innerMaxIter) {
                warning(paste0("Inner optimization did not converge to an optimal solution for rho. Increase iterations to more than ",opt$innerMaxIter),immediate. = TRUE)
                return(list(comb_alpha=numeric(),kern_weight=numeric(),sorted_kern_weight=numeric(),bias=numeric(),activeset=numeric(),history=history,opt=opt))
            }
        }
        ...
        if (length(activeset)>0) {
            ##this follows from the discussion in section 4.2 of Suzuki
            ##(2011) SpicyMKL
            ##see the first pop-up note with the relevant derivations
        ...
        ## these are the actual vectors of kernel weights now
        ## (alpha in in SpicyMKL Tomioka Suzuki JMLR 2011)
        krnlW <- -krnlWMod*matrix(cgamma,N,M,byrow=TRUE)
        krnlWNorm <- kernel.norm(K,krnlW,activeset)$normj

        ## see second paragraph of section 6.1 in
        ## SpicyMKL Tomioka Suzuki JMLR 2011
        rhoMod <- rho - oneN*(sum(rho)/N)
        rhoNorm <- kernel.norm(K,matrix(rhoMod,N,M),activeset)$normj
        ...
        dualGap <- if(is.infinite(primalVal)) Inf else abs(primalVal-dualVal)/abs(primalVal)
        ...
        if ( dualGap < opt$tolOuter && opt$stopDualityGap ) { break }
        ##end Duality Gap stopping ###########################
        if (opt$stopDualityGap &&
            nrow(history)>opt$minIter &&
            (history[step-opt$minIter,"dualityGap"]-dualGap)/history[step-opt$minIter,"dualityGap"]<opt$tolMinIter) {
            warning("The duality gap has been closing very slowly indicating slow convergence.You should examine your kernels for multicollinearity and or change regularization parameters.Alternatively you can increase minIter or decrease tolMinIter.",immediate. = TRUE)
            return(list(comb_alpha=numeric(),kern_weight=numeric(),sorted_kern_weight=numeric(),bias=numeric(),activeset=numeric(),history=history,opt=opt))
            }
        ...
        ## Also Update Augmented Lagrangian coefficients (cgamma and cgammab) ###
        ...
        ## from MATLAB code
        ##hresid  <- (normj*cgamma - pr).^2
        ## i.e. hresid measures which proximal to the norm of
        ## a given kernel's weights show the biggest difference
        ## from the actual norm
        hresid <- sqrt(colSums((krnlWPrInKKT-matrix(rho,N,M))^2))
        maxgap <- 0

        ## tmp is how much cgammab is changing from one iteration to the next,
        ## it is used to determine how quickly to ramp up cgammab
        tmp <- abs(sumrho)
        ##determine how quickly to adjust cgammab
        ##(the augmented Lagrangian coefficient of cb)
        big_cb <- (tmp > cbeta*ck)

        maxgap <- max(tmp,maxgap)

        ##compute which residuals are large enough ro require faster adjustment
        ## of the augmented Lagrangian coefficient cgamma for the respective
        ## kernel norm
        ...
        if (maxgap <= ck) { ck <- maxgap }

        if ( ck < opt$tolOuter && opt$stopIneqViolation ) { break }

        ##end KKT stopping ###########################

        ## update cgammab according to how close we are to an optimal cgammab
        ## (determined by the size of the sumrho update relative to the size of
        ## the gaps between the kernel norm proximals and actual kernel norms -
        ## large update means we are still making big steps (i.e. we can move
        ## faster towards removing the AL term); smaller updates mean we are
        ## still moving in the vicinity of the current solution,
        ## which might be a result of a nearby non-smooth region
        ...
        if (step==opt$outerMaxIter) {
            warning(paste0("Outer optimization did not converge to an optimal solution for alpha and b. Increase iterations to more than ",opt$outerMaxIter),immediate. = TRUE)
            return(list(comb_alpha=numeric(),kern_weight=numeric(),sorted_kern_weight=numeric(),bias=numeric(),activeset=numeric(),history=history,opt=opt))
            }
    ...
    if ( sum_k_weight != 0 ) { kern_weight <- kern_weight/sum_k_weight }

    ## you rescale rho so that the scaling of the kernel weights is negated and the
    ## model can be used directly without consideration about the kernel weight scaling
    ## (which is for visualization/ease of interpretation purposes really)
    ...
    res <- list(comb_alpha=comb_alpha,kern_weight=kern_weight,sorted_kern_weight=sorted_kern_weight,bias=bias,activeset=activeset,idx.train=dimnames(K)[[1]],history=history,opt=opt)
    class(res) <- c("spicer",class(res))
    
    if(opt$includeSubW) res$kern_alpha <- krnlW
...
funceval <- function(loss,normj,yapp,wghts,rho,yrho,sumrho,cgamma,cgammab,cb,pr,C,reg.func){
    val <- switch(loss,
        logit=logit.loss(check.yrho(yrho,wghts),wghts),
        square=square.loss(yapp,wghts,rho))
    ##they use Proposition 1 (eqn 23) in SpicyMKL Tomioka Suzuki
    ##JMLR 2011 to convert Moreau envelope of the convex conjugate
    ##into moreau envelope of norm(alpha+gamma*rho)^2/2 - moreau
    ##envelope of regularization function
    ##the latter can then be evaluated at
    ##prox(norm(alpha+gamma*rho)) as in Boyd Promixal algorithms
    ##monograph, 3.1 , right before eqn 3.2
...
gradient <- function(loss,yapp,wghts,yrho,rho,sumrho,cgamma,cgammab,cb,C,wdot,normj,pr,activeset){
    val <- switch(loss,
        logit=logit.grad(yrho,yapp,wghts),
        square=square.grad(yapp,wghts,rho))
...
hessian <- function(loss,yapp,wghts,yrho,cgamma,cgammab,C,K,normj,wdot,pr,prox.deriv,activeset){
    val <- switch(loss,
        logit=logit.hess(yrho,wghts),
        square=square.hess(length(yapp),wghts))
...
##Primal Obj Funcitons###########################
## z here has the opposite sign to the MATLAB code 
##(but the right one theoretically) - the opposite sign is compensated for by negating
## z in the argument pre-processing
primal.obj <- function(loss,yapp,wghts,z,nactive,reg.func,krnlWNorm,C) {
    primal <- switch(loss,
                  logit=sum(wghts*log(1+exp(-yapp*z))),
                  square=sum(wghts*(yapp-z)^2)*0.5)

    if(nactive>0) {primal <- primal + sum(reg.func(krnlWNorm,C)) }
...
check.yrho <- function(yrho,wghts) {
                     ##this is to make sure yrho falls
                      ##within the required constraint
                      ##-1<-yrho<0,
                      ##which is the negated actual constraint
                      ##0>yrho>1
                      pmin(pmax(yrho,-wghts+1e-07),-1e-7)
                  }


##Loss functions###################################################
##Dual Loss Functions##############################
## all functions below are actually related to the dual of the loss function (aka convex conjugate), 
##i.e. *.loss is the evaluation of the dual of the respective loss,
## *.grad evaluate the gradient of the dual of the loss
## *.hess evaluates the Hessian of the dual of the loss
## the dual variable is rho, which is the lagrangian of the K*alpha+beta (pseudo) constraint - it is Nx1 dimensional (i.e. rho is per-sample, computed across all kernels)

##Logit Loss#######################################

##aka dual/convex conjugate of logit loss
##see see table 1 in Tomioka 2009 - Dual Augmented Lagrangian
## also Suzuki 2011 Spicy MKL eqn(16)
## difference is that negative sign of rho is accounted for in function calculation
...
## different from table 1 in Tomioka 2009 - Dual Augmented Lagrangian
## again because of the substitution of rho for negative rho
##also this is technically -logit.grad, even with the variable substitution!!
logit.grad <- function(yrho,yapp,wghts){
    grad <- yapp*log((wghts+check.yrho(yrho,wghts))/(-yrho))
    return(grad)
}

##remember, the logit uses binary {-1,1} y labels, so the y^2 term that should
##have been in the numerator disappears!!
...
##the proximal operators are vectorized
...
get.reg.funcs <- function(regname=c("l1","elasticnet")){
    ...
    switch(regname,
           l1=list(reg.func=l1.reg,reg.dual=l1.dual,prox=l1.prox,prox.deriv=l1.prox.deriv),
           elasticnet=list(reg.func=elas.reg,reg.dual=elas.dual,prox=elas.prox,prox.deriv=elas.prox.deriv))
...
##aka convex conjugate
##see Suzuki (2011) SpicyMKL -eqn (21)
...
##see table 2 in Tomioka 2009 - Dual Augmented Lagrangian
## can also be derived with Moreau decomposition and projection on the
## Linf-norm ball - see Boyd Proximal Algorithms monograph, section 6.5.1
...
##aka convex conjugate
##see Suzuki(2011) -SpicyMKL eqn(46)
